...
// endereçamento aberto para utilizar tentativa quadrática
// como método de resolução de colisões.

// Código Fonte:

// int hash(int mat, int m, int k) {
//     return (hash_linha(mat, m) + k) % m; // Tentativa (sondagem) linear
// }
...
// Agora com tentativa quadrática
int hash(int mat, int m, int k) {
    return (hash_linha(mat, m) + k * k) % m;
}
